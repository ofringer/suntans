 ...
 * correction method.
 *
 * Added fluxes of w-momentum at the boundaries to the advection terms
 * for w.
 *
...
{
  int flag=0, i, j, jptr, ib, Nc=grid->Nc, Ne=grid->Ne, nf, Nb=grid->edgedist[5]-grid->edgedist[2];
  ...
  *phys = (physT *)SunMalloc(sizeof(physT),"AllocatePhysicalVariables");
  ...
  (*phys)->h = (REAL *)SunMalloc((Nc+Nb)*sizeof(REAL),"AllocatePhysicalVariables");
...
{
  int i, j, k, jptr, Ne=(*grid)->Ne, Nc=(*grid)->Nc, Nb=(*grid)->edgedist[5]-(*grid)->edgedist[2];

  (*grid)->dzz = (REAL **)SunMalloc((Nc+Nb)*sizeof(REAL *),"InitializeVerticalGrid");
  (*grid)->dzzold = (REAL **)SunMalloc(Nc*sizeof(REAL *),"InitializeVerticalGrid");
  (*grid)->gradb = (int *)SunMalloc((2*Ne)*sizeof(int),"InitializeVerticalGrid");

  (*grid)->dv = ReSizeReal((*grid)->dv,Nc+Nb);
  (*grid)->ctop = ReSize((*grid)->ctop,Nc+Nb);
  (*grid)->Nk = ReSize((*grid)->Nk,Nc+Nb);

  for(j=0;j<Ne;j++) {
    (*grid)->gradb[2*j]=(*grid)->grad[2*j];
    (*grid)->gradb[2*j+1]=(*grid)->grad[2*j+1];
  }
  ...
  for(jptr=(*grid)->edgedist[1];jptr<(*grid)->edgedist[2];jptr++) {
    ...
    (*grid)->gradb[2*j+1]=(*grid)->gradb[2*j];
  }
  for(jptr=(*grid)->edgedist[2];jptr<(*grid)->edgedist[5];jptr++) {
    j=(*grid)->edgep[jptr];

    (*grid)->dzz[i]=(REAL *)SunMalloc(((*grid)->Nk[(*grid)->grad[2*j]])*sizeof(REAL),"InitializeVerticalGrid");
    for(k=0;k<(*grid)->Nk[(*grid)->grad[2*j]];k++) {
      (*grid)->dzz[i][k]=(*grid)->dz[k];  
    }
    (*grid)->dv[i]=(*grid)->dv[(*grid)->grad[2*j]];
    (*grid)->ctop[i]=(*grid)->ctop[(*grid)->grad[2*j]];
    (*grid)->Nk[i]=(*grid)->Nk[(*grid)->grad[2*j]];
    (*grid)->gradb[2*j+1]=i++;
  }
}
...
static void UpdateDZ(gridT *grid, physT *phys, int option)
{
  int i, j, k, jptr, ne1, ne2, Nc=grid->Nc, Ne=grid->Ne, Nb=grid->edgedist[5]-grid->edgedist[2], flag;
  REAL z;

  // Set the ghost free-surface values to those specified in boundaries.c
  for(jptr=grid->edgedist[2];jptr<grid->edgedist[5];jptr++) {
    j=grid->edgep[jptr];

    phys->h[grid->gradb[2*j+1]]=phys->boundary_h[jptr-grid->edgedist[2]];
  }
		      ...
		      propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  for(jptr=grid->edgedist[0];jptr<grid->edgedist[1];jptr++) {
	...
	E[j][grid->etop[j]]=1.0/b[grid->etop[j]];
      ...
      phys->D[j]=0;
      for(k=grid->etop[j];k<grid->Nke[j];k++) {
	fluxheight=UpWind(phys->u[j][k],grid->dzz[grid->gradb[2*j]][k],grid->dzz[grid->gradb[2*j+1]][k]);
  ...
  for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
    ...
    for(nf=0;nf<NFACES;nf++) {
      ...
      //      if(grid->gradb[2*ne+1]==-1 || grid->gradb[2*ne+1]>=grid->Nc) printf("ERROR %d!\n",grid->gradb[2*ne+1]);
      for(k=grid->etop[ne];k<grid->Nke[ne];k++) {
	fluxheight=UpWind(phys->u[ne][k],grid->dzz[grid->gradb[2*ne]][k],grid->dzz[grid->gradb[2*ne+1]][k]);
		   ...
		   REAL **src1, REAL **src2, REAL *Ftop, REAL *Fbot, int alpha_top, int alpha_bot) 
{
  ...
  for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
	...
	if(nc1==200) {
	  printf("dz=%f, dzold=%f, u*dt*dzup/A=%f, ddz=%f, s=%f\n",
		 grid->dzz[i][0],grid->dzzold[i][0],
		 (prop->theta*phys->u[ne][0]+(1-prop->theta)*phys->utmp2[ne][0])*grid->df[ne]*prop->dt*grid->dzzold[200][0]/grid->Ac[i],
		 grid->dzz[i][0]-grid->dzzold[i][0],
		 (grid->dzzold[i][0]+
		  (prop->theta*phys->u[ne][0]+(1-prop->theta)*phys->utmp2[ne][0])*grid->df[ne]*
		  prop->dt*grid->dzzold[200][0]/grid->Ac[i])/grid->dzz[i][0]);
	}
      }
    }	
