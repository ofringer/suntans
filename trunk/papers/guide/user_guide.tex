...
% 10 June 2004
%
% $Id: user_guide.tex,v 1.5 2005-07-13 17:07:47 fringer Exp $
...
\subsection*{Nkmax: $\mbox{Nkmax}\ge 1$}
...
\subsection*{rstretch: $-1.1 < \mbox{rstretch} < -1$ or $1 < \mbox{rstretch} < +1.1$}

Employ vertical grid stretching such that the distribution of vertical
grid spacings is given by
\[
\Delta z_k = \mbox{rstretch}\,\Delta z_{k+1}\,,
\]
where $0\le \mbox{k} < \mbox{Nkmax}$.  If $\mbox{rstretch}<0$ then the grid is refined near the
...
\]
The Voronoi points are corrected if $D_g/D_c< VoronoiRatio$, and if this
is the case, they are updated with
\begin{eqnarray}
xv_1 &=& xc + \mbox{VoronoiRatio}(xc_1-xc)\,,\nonumber\\
yv_1 &=& yc + \mbox{VoronoiRatio}(yc_1-xc)\,,\nonumber\\
xv_2 &=& xc + \mbox{VoronoiRatio}(xc_2-xc)\,,\nonumber\\
yv_2 &=& yc + \mbox{VoronoiRatio}(yc_2-xc)\,,\nonumber
\end{eqnarray}
where $xc=(xc_1+xc_2)/2$ and $yc=(yc_1+yc_2)/2$.  Using this correction methodology,
setting VoronoiRatio to 1 moves the Voronoi points to the centroids of the cells.

\subsection*{VoronoiRatio: $0\le\mbox{VoronoiRatio}\le 1$}

Adjustment factor for degenerate triangles.  If $\mbox{VoronoiRatio}=1$, then
all Voronoi points are moved to the centroids, while if $\mbox{VoronoiRatio}=0$, 
then no correction is performed.

\subsection*{vertgridcorrect: Boolean}

For an equispaced vertical grid, the vertical grid spacing is constant and
is given by $\Delta z=D_{max}/\mbox{Nkmax}$, where $D_{max}$ is the maximum depth in the domain.
...
such that $\mbox{Nkmax}=D_{max}/D_{min}$.
...
\subsection*{thetaramptime: $\mbox{thetaramptime}>0$}
...
down to the value of $\theta$ specified below.  The thetaramptime parameter
...
\begin{eqnarray}
\theta(t) &=& \theta_0 + (1-\theta_0)\exp\left(-\frac{t}{\mbox{thetaramptime}}\right)\nonumber\,.
\end{eqnarray}

\subsection*{beta}
			1	\# Expansivity of salt
\subsection*{theta}
			0.55	\# 0: fully explicit, 1: fully implicit
\subsection*{thetaS}
			0.55	\# For scalar advection
\subsection*{thetaB}
			0.55	\# For scalar advection
\subsection*{kappa\_s}
                 1e-4 	\# Vertical mass diffusivity
\subsection*{kappa\_sH}
                1e-4       \# Horizontal mass diffusivity
\subsection*{gamma}
 			1 	\# Coefficient of expansivity of temperature.
\subsection*{kappa\_T}
                 0	\# Vertical thermal diffusivity
\subsection*{kappa\_TH}
                0       \# Horizontal thermal diffusivity
\subsection*{nu}
 			1e-3  	\# Laminar viscosity of water (\verb+m^2 s^-1+)
\subsection*{nu\_H}
 			1e-3    \# Horizontal laminar viscosity of water (\verb+m^2 s^-1+)
\subsection*{tau\_T}
			0 	\# Wind shear stress
\subsection*{z0T}
	                0.0 	\# Top roughness
\subsection*{z0B}
	                0.0	\# Bottom roughness
\subsection*{CdT}
	                0.0 	\# Drag coefficient at surface
\subsection*{CdB}
	                0.005	\# Drag coefficient at bottom
\subsection*{CdW}
			0.005	\# Drag coefficient at sidewalls
\subsection*{turbmodel}
		1	\# Turbulence model (0 for none, 1 for MY25)
\subsection*{dt}
 			0.025	\# Time step
\subsection*{Cmax}
 			1 	\# Maximum permissible Courant number
\subsection*{nsteps}
 			2000    \# Number of steps
\subsection*{ntout}
   		50  	\# How often to output data
\subsection*{ntprog}
   		5 	\# How often to report progress (in %)
\subsection*{ntconserve}
 		1	\# How often to output conserved data
\subsection*{nonhydrostatic}
		1	\# 0 = hydrostatic, 1 = nonhydrostatic
\subsection*{cgsolver}
		1	\# 0 = GS, 1 = CG
\subsection*{maxiters}
		1000	\# Maximum number of CG iterations
\subsection*{qmaxiters}
		2000	\# Maximum number of CG iterations for nonhydrostatic pressure
\subsection*{qprecond}
		2	\# 1 = preconditioned, 0 = not preconditioned
\subsection*{epsilon}
			1e-20 	\# Tolerance for CG convergence
\subsection*{qepsilon}
		1e-5	\# Tolerance for CG convergence for nonhydrostatic pressure
\subsection*{resnorm}
			1	\# Normalized or non-normalized residual
\subsection*{relax}
