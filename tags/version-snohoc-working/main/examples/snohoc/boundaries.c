...
void BoundaryScalars(gridT *grid, physT *phys, propT *prop) {
  ...
  for(jptr=grid->edgedist[2];jptr<grid->edgedist[3];jptr++) {
      ...
      if(grid->xv[ib]>0.0)   // Riverine
	{

	  for(k=grid->ctop[ib];k<grid->Nk[ib];k++) {
	    phys->boundary_T[jptr-grid->edgedist[2]][k] = 0.6;
	...
	{
	  for(k=grid->ctop[ib];k<grid->Nk[ib];k++) {
	    if(phys->u[j][k]>0) { //coming in
	      phys->boundary_T[jptr-grid->edgedist[2]][k] = 0.0;
	      phys->boundary_s[jptr-grid->edgedist[2]][k] = 32;
	    }
	    else {
...
void BoundaryVelocities(gridT *grid, physT *phys, propT *prop, int myproc) {
  ...
  for(jptr=grid->edgedist[2];jptr<grid->edgedist[3];jptr++) {
	...
	if(grid->yv[ib]>0) 
	  phys->boundary_v[jptr-grid->edgedist[2]][k]=0.075*sin(prop->omega*prop->rtime);
	else
	  phys->boundary_v[jptr-grid->edgedist[2]][k]=-0.075*sin(prop->omega*prop->rtime);
	...
	phys->boundary_w[jptr-grid->edgedist[2]][k]=0.5*(phys->w[ib][k]+phys->w[ib][k+1]);
      ...
      for(k=grid->etop[j];k<grid->Nke[j];k++) {
	phys->boundary_u[jptr-grid->edgedist[2]][k]=0;
	phys->boundary_v[jptr-grid->edgedist[2]][k]=0.1;
