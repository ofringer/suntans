...
cdp = load('./dataXZ/celldata.dat.0'); vx = cdp(:,1); vy = cdp(:,2);
z = getz(load('./dataXZ/vertspace.dat')); dz = max(z(1:end-1)-z(2:end));

% setup profiles
N = 100; dx=1/N;
xprof = linspace(dx/2,1-dx/2,N);
yprof = mean(vy)*ones(1,N);
zprof = -0.5*ones(1,N);
Iset1 = [1:N];
...
yprof = [yprof mean(vy)*ones(1,N)];
zprof = [zprof -linspace(dz/2,1-dz/2,N)];
Iset2 = [N+2:length(xprof)];

% load in velocity information corresponding to (vx, vy)
fid = fopen('./dataXZ/u.dat.0','rb');
Nkmax = 128; Nc = length(vx); arraysize = Nc*3*Nkmax;
...
figure()
hold on
h11 = line(-zprof(Iset2),-uprof(Iset2),'Color','k');
...
ax2 = axes('Position',get(ax1,'Position'), ...
    ...
    'Color','none',...
    'XColor','b', 'Ycolor','b');
h21 = line(-wprof(Iset1(end:-1:1)),xprof(Iset1),'Color','b','Parent',ax2);
